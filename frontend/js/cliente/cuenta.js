document.addEventListener("DOMContentLoaded", function () {
  ...
  (async function cargar() {
    ...
    let misReservas = [];
    try {
      const datos = await apiGet('/api/reservas/mias');
      misReservas = datos.reservas;
    } catch (error) {
    ...
    if (misReservas.length === 0) {
      ...
      reservasHtml = misReservas.map(function (r) {
        const asientos = r.asientos.map(function (asiento) {
          return asiento + " (Piso " + (asiento <= 20 ? 1 : 2) + ")";
        }).join(", ");
        ...
        return (
          '<article class="viaje-item">' +
            ...
            '<div style="text-align:right;">' +
              '<div class="viaje-precio">S/ ' + Number(r.total).toFixed(2) + '</div>' +
